...
function parseTime(time) {
    ...
    let match = time.toLowerCase().replace(/[^a-z0-9\.]/g, "").match(/(?:(\d+(?:\.\d+)?)(?:days?|d))?(?:(\d+(?:\.\d+)?)(?:hours?|hrs?|h))?(?:(\d+(?:\.\d+)?)(?:minutes?|mins?|m\b))?(?:(\d+(?:\.\d+)?)(?:seconds?|secs?|s))?(?:(\d+(?:\.\d+)?)(?:milliseconds?|ms))?/);
...
function parseDate(date) {
    let match = date.match(/(\d{4})(?:\-)(\d{2})(?:\-)(\d{2})(?:\s+)(\d{2})(?:\:)(\d{2})(?:\:)(\d{2})/);
    ...
    if (match) {
        ...
        let expectedDate = new Date(year, month - 1, day, hour, min, sec),
 ...
 * @return {Int}               [id]
 */
export function repeat(callback, time) {
    let ms = parseTime(time),
 ...
 * @return {Int}               [id]
 */
export function until(callback, date) {
...
export function frame(callback) {
    let timerId = requestFrame(callback);
    return timerId;
}
